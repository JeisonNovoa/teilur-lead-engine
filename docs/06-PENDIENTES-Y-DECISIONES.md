...
| 2026-06-01 | **ARREGLO de enriquecimiento.** Bug: los leads tenían email pero NO datos de empresa (website/industria/tamaño). Causa: `enrichLead` ignoraba `person.organization` que `people/match` ya devuelve gratis. Arreglado para aprovechar esos datos. Nuevo comando `npm run reenrich` re-completa leads viejos. Los 19 actuales re-enriquecidos + re-calificados (mensajes ahora mucho más específicos). |
